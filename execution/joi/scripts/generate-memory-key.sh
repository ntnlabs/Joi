...
#
# Usage: sudo ./generate-memory-key.sh [SERVICE_USER]
#
# This creates /etc/joi/memory.key with a random 64-character key.
# The file is owned by SERVICE_USER (default: joi) with mode 600.
...
if ! id "$SERVICE_USER" &>/dev/null; then
    ...
    echo "Usage: sudo ./generate-memory-key.sh [SERVICE_USER]"
    exit 1
fi
